...
class PickColorCommand(sublime_plugin.TextCommand):
	...
	def run(self, edit, paste = True):
		start_color = self.get_selected(edit) or "#000000"	# blacky
		color = self.__hexstr_to_bgr(start_color)
		print("pick color: %s (#%06x)" % (start_color, color))

		fg, bg = self.pick(color)
		if(fg):
			color = self.__bgr_to_hexstr(fg)
			print("put  color: %s (#%06x)" % (color, fg))
			if paste:
				self.put_selected( edit, color )


	def pick(self, initial = 0):
		import ctypes
		...
		dll = ctypes.cdll.LoadLibrary(r"d:\tools\graphics\ColorPicker.dll")
		fn = dll.PickColor
		...
		fg = c_uint32(initial)		# black
		bg = c_uint32(0xFFFFFFFF)	# white
		re = fn(None, byref(fg), byref(bg))
		if re == IDOK:
			return fg.value & 0xFFFFFF, bg.value & 0xFFFFFF
		return None, None


	def get_selected(self, edit):
		""" returns currently selected color in '#FFFFFF' format """
	...
	def __bgr_to_hexstr(self, bgr):
		""" returns color as '#RRGGBB' """
